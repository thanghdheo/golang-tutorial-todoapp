...
func UpdateTodoAsDelete(db *gorm.DB) gin.HandlerFunc{
	return func(ctx *gin.Context) {
		...
		biz := todobiz.NewUpdateTodoAsDelBiz(storage)

		data.Deleted = true
		
		if err := biz.UpdateTodoAsDelete(ctx.Request.Context(), id , &data); err != nil {
			ctx.JSON(http.StatusBadRequest, gin.H{"error": err.Error()})
			return
		}
